...
-----
  ...
  python pideeponet_2d_dirichlet_v0_cuda_test_bayesian_output_scale_1.py --bayes_init_res 1.0 --bayes_init_d 100.0 --bayes_init_neu 10.0
"""
...
def plot_comsol_prediction_error_grid(xy, u_comsol, predictions, v0_values, rel_errors,
                                      save_dir, fname="comsol_prediction_error_grid.png"):
    triang = _tri(xy)
    n_rows = len(v0_values)
    fig_h = max(10.0, 1.25 * n_rows)
    fig, axes = plt.subplots(n_rows, 3, figsize=(9.5, fig_h), squeeze=False)

    for r, (v0, u_ref, u_pred, rel) in enumerate(zip(v0_values, u_comsol.T, predictions, rel_errors)):
        err = np.abs(u_ref - u_pred)
        vmin, vmax = u_ref.min(), u_ref.max()
        err_max = max(float(err.max()), 1e-12)

        panels = [
            (u_ref, "COMSOL", "jet", vmin, vmax),
            (u_pred, "Prediction", "jet", vmin, vmax),
            (err, "|Error|", "hot", 0.0, err_max),
        ]
        for c, (vals, title, cmap, lo, hi) in enumerate(panels):
            ax = axes[r, c]
            contour = ax.tricontourf(triang, vals, levels=48, cmap=cmap, vmin=lo, vmax=hi)
            ax.set_aspect("equal")
            ax.set_xticks([])
            ax.set_yticks([])
            if r == 0:
                ax.set_title(title)
            if c == 0:
                ax.set_ylabel(f"v0={v0:.1f}", rotation=0, ha="right", va="center", labelpad=28)
            if c == 1:
                ax.text(0.5, -0.12, f"rel-L2={rel*100:.2f}%", transform=ax.transAxes,
                        ha="center", va="top", fontsize=7)
            cbar = fig.colorbar(contour, ax=ax, fraction=0.046, pad=0.02)
            cbar.ax.tick_params(labelsize=6)

    fig.suptitle("COMSOL vs Prediction vs Error", y=0.995, fontsize=12)
...
def main():
    ...
    parser.add_argument("--train_step",      type=float, default=1.0,
                        help="Step size between training v0 values (e.g. 1.0 = integers 0,1,...,20; "
                             "0.5 = all 41 samples; default 1.0 = train integers and test half-steps).")
    ...
    history = trainer.run(epochs=args.epochs, verbose_freq=1, log_dir=args.log_dir,
    ...
    plot_comsol_prediction_error_grid(
        xy, u_comsol, predictions, v0_values, rel_errors, save_dir=args.log_dir)
