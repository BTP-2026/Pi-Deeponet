...
def solve_poisson_2d_mixed(x, y, f, bc_values, bc_types, f_scale=100.0):
    """
    2D Poisson solver with mixed boundary conditions (Dirichlet + Neumann).

    PDE:  -nabla^2 u = f_scale * f   on [0,1] x [0,1]

    Parameters
    ----------
    x, y       : 1-D grid arrays
    f          : (Ny, Nx) source field  OR  scalar
    bc_values  : dict  {'left': array/scalar, 'right': ..., 'bottom': ..., 'top': ...}
                 For Dirichlet: value of u.  For Neumann: value of du/dn (outward normal).
    bc_types   : dict  {'left': 'dirichlet', 'right': 'dirichlet',
                        'bottom': 'neumann', 'top': 'neumann'}
    f_scale    : multiplier in PDE  (default 100)

    Returns
    -------
    u : (Ny, Nx) solution array
    """
    Nx = len(x)
    Ny = len(y)
    dx = x[1] - x[0]
    ...
    for side, val in bc_values.items():
        if np.isscalar(val):
            ...
            bc_arrays[side] = np.asarray(val, dtype=np.float64)

    # Apply Neumann BCs FIRST (so Dirichlet can overwrite at shared corners)
    for side in ['bottom', 'top', 'left', 'right']:
        if bc_types.get(side, 'dirichlet') != 'neumann':
            continue
        idx = bc_indices[side]
        val = bc_arrays[side]
        if side in ['bottom', 'top']:
            # du/dy = val  →  (Dy_2d / (2*dy)) @ u = val
            for k, node_idx in enumerate(idx):
                L_sys[node_idx, :] = Dy_2d[node_idx, :] / (2.0 * dy)
                b[node_idx] = val[k]
        else:  # left, right
            # du/dx = val  →  (Dx_2d / (2*dx)) @ u = val
            for k, node_idx in enumerate(idx):
                L_sys[node_idx, :] = Dx_2d[node_idx, :] / (2.0 * dx)
                b[node_idx] = val[k]

    # Apply Dirichlet BCs SECOND (overwrites corners shared with Neumann sides)
    for side in ['left', 'right', 'bottom', 'top']:
        if bc_types.get(side, 'dirichlet') != 'dirichlet':
            continue
        idx = bc_indices[side]
        L_sys[idx, :] = I_sp[idx, :]
        b[idx] = bc_arrays[side]
